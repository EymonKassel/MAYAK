...
public class Magnet : MonoBehaviour {
    ...
    public MagneticGun MagneticGun;
    ...
    private void LinePointToFirePoint() {
        for ( int i = 0; i < _percision; i++ ) {
            _lineRenderer.SetPosition(i, MagneticGun.FirePoint.position);
        }
    }
    ...
    private void DrawRope() {
        if ( !_straightLine ) {
            if ( _lineRenderer.GetPosition(_percision - 1).x != MagneticGun.GrapplePoint.x ) {
                DrawRopeWaves();
            } else {
                _straightLine = true;
            }
    ...
    private void DrawRopeWaves() {
        for ( int i = 0; i < _percision; i++ ) {
            ...
            Vector2 offset = Vector2.Perpendicular(MagneticGun.DistanceVector).normalized * RopeAnimationCurve.Evaluate(delta) * _waveSize;
            Vector2 targetPosition = Vector2.Lerp(MagneticGun.FirePoint.position, MagneticGun.GrapplePoint, delta) + offset;
            Vector2 currentPosition = Vector2.Lerp(MagneticGun.FirePoint.position, targetPosition, RopeLaunchSpeedCurve.Evaluate(_moveTime) * _ropeLaunchSpeedMultiplayer);
    ...
    private void DrawRopeNoWaves() {
        ...
        _lineRenderer.SetPosition(0, MagneticGun.GrapplePoint);
        _lineRenderer.SetPosition(1, MagneticGun.FirePoint.position);
    }
}
